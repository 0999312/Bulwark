...
| 反馈修复 | done | `scripts/systems/fx_burst.gd`（爆炸池）、`tools/capture_runner.gd`（showcase 不再污染共享 SpriteFrames）、`scenes/enemy/enemy_view.gd`、`scenes/base/{turret.tscn,turret_view.gd}`、`scenes/player/player_view.gd`、`scenes/vfx/{enemy_projectile,turret_tracer}.gd`、`scenes/ui/hud.gd` | ①爆炸 5 帧不再循环（共享 SpriteFrames loop 被 showcase 污染已修复；池节点播完即隐藏），爆炸/死亡粒子缩放下调；②炮塔放大至 1.15、弹体/枪口焰放大，素材默认朝上 → 递增对齐弹道（+PI/2 修正炮口与弹迹垂直）；③HUD 方向文字雷达/罗盘箭头已从游戏剔除（仅保留数量档+精英标记；纯函数保留供测试）；GUT 312/312、冒烟 EXIT=0、截图复核通过 |
| 17–20（P2） | pending | — | 未开始（无尽模式/meta/章间三选一/叙事，可选） |
