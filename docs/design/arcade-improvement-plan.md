...
| 横幅排版反馈 | done | `scenes/ui/hud.tscn`、`scenes/ui/hud.gd` | 横幅改为**内容自适应**（BannerTop(Margin) → Center → PanelContainer 包裹 VBox）：宽度随实际文本伸缩，标题 26px + 便签 16px 两行完整显示；StyleBoxFlat_banner_bg 加内容边距（左右 26 / 上下 12），文字与边框间隙舒适；1280 截图复核通过 |
| 弹药补给修复 | done | `scripts/systems/game_session.gd`（`_apply_power_up` / `_shop_effect_handler`）、`resources/shop/items/shop_ammo_crate.tres`、`tools/update_locales.py`、`tests/integration/test_m1_game_session.gd` | 泛用补给修复：弹药箱（波中道具 & 商店固定物资）同时补充 子弹 +30 / 能量 +15，能量武器不再缺弹；新增回归 1 个（shop 能量 +15）+1 个（波中弹药箱能量 +15）；GUT 320/320 |
