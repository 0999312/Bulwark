extends GutTest
## M2 双人强度缩放（EnemyController hp_scale）：血量乘数生效、默认参数不破坏单机路径
...
func test_hp_scale_multiplies_health() -> void:
	...
	var runner := EnemyController.new(data, 1.6)
...
func test_hp_scale_guard_against_non_positive() -> void:
	var data := _make_data(30.0)
	var runner := EnemyController.new(data, 0.0)
	assert_gt(runner.health, 0.0, "非正缩放被防御钳制")
	var runner2 := EnemyController.new(data, -2.0)
...
func test_scaled_runner_keeps_behaviour() -> void:
	...
	assert_eq(runner.state, EnemyController.State.ATTACK, "行为 FSM 不受缩放影响")
